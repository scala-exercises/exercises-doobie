...
import doobie_lib.UpdatesSectionHelpers.Person
...
object UpdatesSection extends FlatSpec with Matchers with Section {
  ...
  def insertOneRow(res0: Int) = {
    ...
    val insertedRows = databaseBlock(insert1("John", Option(35)).run).unsafeRunSync()
  ...
  def insertSeveralRowsWithForComprehension(res0: Int) = {
    ...
    val insertedRows = databaseBlock(rows).unsafeRunSync()
  ...
  def insertSeveralRowsWithApplicativeFunctor(res0: Int) = {
    ...
    val insertedRows =
      databaseBlock((insertedOnePerson, insertedOtherPerson).mapN(_ + _)).unsafeRunSync()
  ...
  def insertSeveralRowsWithTraverse(res0: Int) = {
    ...
    val insertedRows =
      databaseBlock(people.traverse(item => (insert1 _).tupled(item).run)).unsafeRunSync()
  ...
  def updateExistingRow(res0: Int, res1: Int, res2: Int) = {
    ...
    val (insertedRows, updatedRows, person) = databaseBlock(result).unsafeRunSync()
  ...
  def retrieveInfo(res0: String, res1: Int) = {
    ...
    val person = databaseBlock(insert2_H2("Ramone", Option(42))).unsafeRunSync()
  ...
  def batchUpdates(res0: Int) = {
    ...
    val insertedRows = databaseBlock(insertMany(data)).unsafeRunSync()
