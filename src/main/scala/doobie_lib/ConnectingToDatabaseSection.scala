...
object ConnectingToDatabaseSection
  ...
  def constantValue(res0: Int) =
    trans.use(42.pure[ConnectionIO].transact[IO]).unsafeRunSync() should be(res0)
  ...
  def constantValueFromDatabase(res0: Int) =
    trans.use(sql"select 42".query[Int].unique.transact[IO]).unsafeRunSync() should be(res0)
  ...
  def combineTwoPrograms(res0: (Int, Int)) = {
    ...
    trans.use(largerProgram.transact[IO]).unsafeRunSync() should be(res0)
  }
  ...
  def combineTwoProgramsWithApplicative(res0: Int) = {
    ...
    trans.use((oneProgram, anotherProgram).mapN(_ + _).transact[IO]).unsafeRunSync() should be(res0)
  }
}
