...
object SelectingDataSection extends FlatSpec with Matchers with Section {
  ...
  def selectUniqueCountryName(res0: String) = {
    ...
    val countryName =
      databaseBlock(sql"select name from COUNTRY where code = 'ESP'".query[String].unique)
        .unsafeRunSync()
  ...
  def selectOptionalCountryName(res0: Option[String]) = {
    ...
    val maybeCountryName =
      databaseBlock(sql"select name from country where code = 'ITA'".query[String].option)
        .unsafeRunSync()
