...
/**
 * ==About Exceptions==
 * Exceptions are a fact of life when interacting with databases, and they are largely
 * nondeterministic; whether an operation will succeed or not depends on unpredictable factors like
 * network health, the current contents of tables, locking state, and so on. So we must decide
 * whether to compute everything in a disjunction like `EitherT[ConnectionIO, Throwable, A]` or
 * allow exceptions to propagate until they are caught explicitly. doobie adopts the second
 * strategy: exceptions are allowed to propagate and escape unless handled explicitly (exactly as IO
 * works). This means when a doobie action (transformed to some target monad) is executed,
 * exceptions can escape.
 *
 * There are three main types of exceptions that are likely to arise:
 *
 * '''1.''' Various types of `IOException` can happen with any kind of I/O, and these exceptions
 * tend to be unrecoverable.
 *
 * '''2.''' Database exceptions, typically as a generic `SQLException` with a vendor-specific
 * `SQLState` identifying the specific error, are raised for common situations such as key
 * violations. Some vendors (PostgreSQL for instance) publish a table of error codes, and in these
 * cases doobie can provide a matching set of exception-handling combinators. However in most cases
 * the error codes must be passed down as folklore or discovered by experimentation. There exist the
 * XOPEN and SQL:2003 standards, but it seems that no vendor adheres closely to these
 * specifications. Some of these errors are recoverable and others aren’t.
 *
 * '''3.''' doobie will raise an `InvariantViolation` in response to invalid type mappings, unknown
 * JDBC constants returned by drivers, observed `NULL` values, and other violations of invariants
 * that doobie assumes. These exceptions indicate programmer error or driver non-compliance and are
 * generally unrecoverable.
 *
 * ==MonadError and Derived Combinators==
 * All '''doobie''' monads have associated instances of the `Async` instance, which extends
 * `MonadError[?[_], Throwable]`. This means `ConnectionIO`, etc., have the following primitive
 * operations:
 *
 ...
 * }}}
 *
 * From the `.attempt` and `fail` combinators we can derive many other operations, as described in
 * the Cats documentation. In addition doobie provides the following specialized combinators that
 * only pay attention to `SQLException`:
 *
...
object ErrorHandlingSection extends AnyFlatSpec with Matchers with Section {
   ...
   * }}}
   *
   * The following exercises will try to insert two people with the same name. The second operation
   * will fail with a unique constraint violation. Let's see how we can avoid this error by using
   * several combinators.
   *
   * A first approach could be to specify the `Throwable` that we want to trap by using `attemptSql`
   * combinator.
   *
  ...
  /**
   * Finally we can recover from an error with a new action by using `exceptSqlState`. In this case,
   * if the name already exists, we'll insert the person with a different name.
   */
